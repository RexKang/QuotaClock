...
- **设置分两个标签**：「平台管理」（平台 + API Key 列表/采集与服务参数）与「账户安全」（鉴权模式/改密码/登录态）
...
4. 浏览器打开 `http://127.0.0.1:8787` → 「设置」→「登录」（默认密码 `Quota@2026090S`）→
   从下拉里选平台、填入 API Key → 保存，采集器即刻开始拉取。设置面板分「平台管理」与「账户安全」两个标签。
...
> v0.2.5 起**只能从这 4 个内置平台里选**，界面上只填 API Key / Cookie：接口地址、请求路径与鉴权方式由程序内置（各家的返回结构需要配套解析，自填地址会「接口通了但字段认不出来」）。同一平台可以加多个 Key——采集会按「基准间隔 ÷ Key 数」把它们的请求等分错开，避免互相冲突。
...
| `http://127.0.0.1:8787/opencode` | 平台 `opencode` | 官方用量接口；v0.1 的 Cookie 登录态无法转换为 API Key，**token 未迁移**——到控制台生成 API Key 后在设置页录入 |
...
- **回退旧版本**：v0.2.5 之前的二进制读 version 4 的 `config.json` 会拒绝启动（提示「配置来自更新版本程序」）；回退需先把配置改回旧结构（version 3 + `id`/`base_url`/`paths` 形态）或删除配置文件重新录入
